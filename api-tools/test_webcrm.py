...
if __name__ == "__main__":
    ...
    # Test 2: Fetch ALL organisations
    # -------------------------------------------------
    print("\n" + "-"*60)
    print("Test 2: Fetch ALL Organisations")
    print("─"*60)
    organisations = client.get_organisations(page=1, size=25)  # Max 250 per page
    
    if organisations:
        print(f"\n📋 Found {len(organisations)} companies:")
        for i, org in enumerate(organisations, 1):
            print(f"   {i}. {org.get('OrganisationName', 'N/A')} (ID: {org.get('OrganisationId', 'N/A')})")
